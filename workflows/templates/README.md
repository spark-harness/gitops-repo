...
`github-image-release` 是镜像构建、扫描和 GitOps promotion 的标准模板。

它不是什么：它不是业务仓构建脚本，也不保存 registry 密钥或生产凭据。

它是什么：它定义 Argo CI 在收到 GitHub 事件后如何取源码、构建镜像、扫描镜像、回写 GitHub 状态，并在主干发布时把 GitOps 环境 overlay 更新到不可变 digest。
...
- PR 构建推送临时 tag：`pr-<pr-number>-<short-sha>`。
- 非 PR 构建推送主干 tag：`sha-<short-sha>`。
- 环境部署只引用 digest：`<image>@sha256:<digest>`。
- GitOps overlay 的 image 更新只发生在 `promote=true` 的 workflow 中。
