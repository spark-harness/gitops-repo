...
push 时创建一个按 commit short SHA 命名的 `github-image-release` workflow，由模板内部的发布计划受控构建、
扫描 `applicant-api`、`fides-bff`、`fides`、`quote-api`、`origination-api`，再一次性更新 GitOps digest。
